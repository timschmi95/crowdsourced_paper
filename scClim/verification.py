...
def plot_measures(df,x,ax,npred=None):
    ...
    x : pandas.index
        indices of the dataframe to be plotted (intensity thresholds)
    ax : matplotlib.pyplot.axis
        plotting axis
    npred : int, optional
        Number of overal predictions in the verification. The default is None.
    ...
    h7,=ax.plot(x,df.CSI[x],color='k',linewidth=lw, linestyle = 'dashdot', label = 'CSI')
...
def get_shifted_corr(arr1,arr2,valid_points,shifts_x,shifts_y,return_type='df',
    ...
    Args:
        ...
        opt_type (str, optional): Whether to optimize correlation or HSS. Defaults to 'corr'.
        HSS_thresh_arr1 (int, optional): HSS threshold to optimize. Defaults to 35.
...
def get_best_shift_per_date(ds,var_to_shift,var_stationary,ds_valid_points,
    ...
    if var_to_shift in ['MESHS','dBZ','MESH']: #fill NaN values with 0
...
def get_shifted_MESHS(df_shifts,haz_data,haz_var:str='MESHS'):
    ...
    for date in df_shifts.index:
        if not df_shifts.loc[date,['x','y']].isna().any():
            ...
            #Note: for the same results, with explicit dimension handling use the
            # line below. For consistency with the get_shifter_corr function, we use np.roll()
            # meshs_shifted['MESHS'].loc[{'date':date}]= meshs_shifted['MESHS'].loc[{'date':date}].shift({'chy':x_shift,'chx':y_shift})
