...
def grid_crowd_source(crowd_source_data,pop_path,date_sel='20210628',
    ...
    """function to grid crowd-sourced data (adapted from Killian's code)
    ...
    if date_sel == 'all':
        ...
        dates = [d.strftime('%Y%m%d') for d in sel_dates if d.year >=2017] #only select dates after 2017 (when reporting categories changed)
...
def group_to_cell(cell,gdf_exp,aggfunc,count_var,plot=False):
    ...
    merged = gpd.sjoin(gdf_exp, cell_out, how='left', predicate='within') #older gpd version: op='within'
...
def create_empty_grid(epsg=2056, cell_size = 1000, extent = None):
   ...
   TODO: could be moved to scClim.util.both but did not work so it remains here for now.
...
def gridded_data_from_agro_damages_and_exposure(exposure_gdf, damages_gdf, return_type = 'xarray',
                                                gridsize_m = 1000):
    """ creates a gridded dataset, based on exposure gdf and a damages gdf, of the following variables
            n_fields_dmg: number of damaged fields
            mdd: mean damage degree of the damaged fields
            n_fields_exp: number of exposed fields
            area_ha_exp: area of exposed fields
            paa: percent of assets affected (fraction of fiels affected)

    Parameters
    ----------
    exposure_gdf : pandas.GeoDataFrame
        geodataframe of exposure, requires the following columns 'value' (number of fields),
        'area_ha' (total area in ha), and 'geometry'
    damages_gdf : pandas.GeoDataFrame
        geodataframe of damages, requires columns 'Ertragsverlust (%)' (harvest loss in %) and 'geometry'
    return_type : str, optional
        which data type to return, supported are 'xarray' or 'gdf'. The default is 'xarray'.
    gridsize_m : int, optional
        gridsize of the output grid in meters. The default is 1000.
    Returns
    -------
    cell_xarray : xarray.Dataset
        Dataset with gridded data on a regular lat lon grid
        (coordinates are chx and chy, Swiss coordinates LV95)
    cell_out: pandas.GeoDataFrame
        GeoDataFrame with gridded data (coordinates are chx and chy, Swiss coordinates LV95)
    """

    #damages_gdf=damages_exp_dict[pd.Timestamp('2021-06-28 00:00:00')].gdf
    #reduce damages gdf

    damages_gdf=damages_gdf[['Ertragsverlust (%)','geometry']]


    #get cell and crs
    cell, crs, extent = create_empty_grid(epsg=2056, cell_size=gridsize_m)

    #merge the grid cells to the subset
    merged_exp = gpd.sjoin(exposure_gdf, cell, how='left')

    merged = gpd.sjoin(damages_gdf, cell, how='left')
    merged['n_fields'] = 1

    #dissolve the subset in the grid cells (count, sum and mean, depending on variable)
    dissolve = merged.dissolve(by="index_right", aggfunc={
        "n_fields": "count",
        "Ertragsverlust (%)": "mean",
        },
    )
    ...
    #dissolve the subset in the grid cells (count, sum and mean, depending on variable)
    dissolve_exp = merged_exp.dissolve(by="index_right", aggfunc={
        "n_fields": "sum",
        "area_ha": "sum",
        },
    )

    #fill cell with required data
    cell.loc[dissolve.index, 'n_fields_dmg'] = dissolve['n_fields'].values
    #cell.loc[dissolve.index, 'mdd'] = dissolve[('Ertragsverlust (%)')].values
    cell.loc[dissolve_exp.index, 'n_fields_exp'] = dissolve_exp[('n_fields')].values
    cell.loc[dissolve_exp.index, 'area_ha_exp'] = dissolve_exp[('area_ha')].values


    if return_type == 'xarray':

        # transform cell for proper execution of centroid computation
        #cell_out=cell.to_crs(crs=CRS.from_epsg(int(2056)))
        cell['geometry'] = cell.geometry.centroid
        cell["chx"] = cell.geometry.x
        cell["chy"] = cell.geometry.y
        cell = cell.round({'chx': 0, 'chy': 0})
        cell['geometry'] = gpd.points_from_xy(cell.chx, cell.chy)

        #get lat lon values
        geometry_latlon=cell['geometry'].to_crs(crs=CRS.from_epsg(int(4326)))
        cell["lon"]=geometry_latlon.geometry.x
        cell["lat"]=geometry_latlon.geometry.y

        cell_multiindex = cell.drop(columns='geometry').set_index(['chy','chx'])
        cell_xarray=cell_multiindex.to_xarray().set_coords(('chy','chx'))
...
def aggregate_exposure(exposure_gdf, gridsize_m = 1000):
    """ creates a gridded dataset, based on exposure gdf and a damages gdf, of the following variables
            n_fields_dmg: number of damaged fields
            mdd: mean damage degree of the damaged fields
            n_fields_exp: number of exposed fields
    ...
    cell.loc[dissolve_exp.index, 'n_fields'] = dissolve_exp[('n_fields')].values
    cell.loc[dissolve_exp.index, 'area_ha'] = dissolve_exp[('area_ha')].values
    cell.loc[dissolve_exp.index, 'value'] = dissolve_exp[('area_ha')].values
